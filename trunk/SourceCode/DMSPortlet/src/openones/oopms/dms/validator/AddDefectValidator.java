...
public class AddDefectValidator extends BaseValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        ValidationUtils.rejectIfEmptyOrWhitespace(errors, "description", "NotEmpty.defect.description");

        if (bean.getTitle().length() > 225 || bean.getTitle().length() < 10) {
            errors.rejectValue("title", "FieldLength.defect.title");
        }
        
        if (bean.getDescription().length() > 1800) {
