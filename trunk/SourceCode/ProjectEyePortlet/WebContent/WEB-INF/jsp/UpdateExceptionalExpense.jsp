...
function fnFeaturesInit ()
{
	...
	  });
	  $( "#datepicker1" ).datepicker({
	        ...
	        yav.init('${portletNamespace}UpdateExceptionalExpense', rules);
            ...
            document.getElementById('description').innerHTML = "${UpdateExceptionalExpenseForm.description}";
	});
...
<title>Update Exceptional Expense</title>	
</head>
...
<div  class="container"><%@ include file="/WEB-INF/jsp/Menu.jsp" %>
  ...
  <!-- begin .navigator -->
   ...
   <div class="fl-widget-titlebar titlebar portlet-titlebar">
    	<br><p class="title" id="headerDuyND">Update Exceptional Expense</p>
    </div>


	<portlet:actionURL var="formAction">
  		<portlet:param name="action" value="UpdateExceptionalExpense" />
  		<portlet:param name="projectId" value="${projectId}" />
  		<portlet:param name="oopmsExceptionalCostId" value="${oopmsExceptionalCostId}" />
	</portlet:actionURL>
	<portlet:renderURL var="renderAction">
    		<portlet:param name="action" value="GoCostManagement" />
        	<portlet:param name="projectId" value="${projectId}" />
  	</portlet:renderURL>
<form:form name="${portletNamespace}UpdateExceptionalExpense" commandName="UpdateExceptionalExpenseForm" method="post" action="${formAction}">
...
<table class="display dataTable" id="mainTable2" cellpadding="0" cellspacing="0" border="0">	
   ...
   <c:forEach var="dailyExpense" items="${DailyExpenseList}">
	...
	<button type="button" class="button blue small" onclick='submitAction2("${portletNamespace}UpdateExceptionalExpense", "${formAction}")'>Update</button>
	<button type="reset" class="button blue small">Reset</button>
	<button type="button" class="button blue small" onclick='submitAction("${portletNamespace}UpdateExceptionalExpense", "${renderAction}")'>Cancel</button>
</form:form>
