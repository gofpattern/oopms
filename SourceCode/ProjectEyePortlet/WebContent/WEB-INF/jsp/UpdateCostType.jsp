...
<script type="text/javascript">
    $(document).ready(function() {
    		document.getElementById('description').innerHTML = "${UpdateCostTypeForm.description}";
	        yav.init('${portletNamespace}UpdateCostType', rules);
	});
...
<div  class="container"><%@ include file="/WEB-INF/jsp/Menu.jsp" %>
  ...
  <!-- begin .navigator -->
   ...
   <div class="fl-widget-titlebar titlebar portlet-titlebar">
    	<br><p class="title" id="headerDuyND">Update Cost Type</p>
    </div>


	<portlet:actionURL var="formAction">
  		<portlet:param name="action" value="UpdateCostType" />
  		<portlet:param name="projectId" value="${projectId}" />
  		<portlet:param name="oopmsCostTypeId" value="${oopmsCostTypeId}" />
	</portlet:actionURL>
	<portlet:renderURL var="renderAction">
    		<portlet:param name="action" value="GoCostManagement" />
        	<portlet:param name="projectId" value="${projectId}" />
  	</portlet:renderURL>
<form:form name="${portletNamespace}UpdateCostType" commandName="UpdateCostTypeForm" method="post" action="${formAction}">
	...
	<button type="button" class="button blue small" onclick='submitAction2("${portletNamespace}UpdateCostType", "${formAction}")'>Update</button>
	<button type="reset" class="button blue small">Reset</button>
	<button type="button" class="button blue small" onclick='submitAction("${portletNamespace}UpdateCostType", "${renderAction}")'>Cancel</button>
</form:form>
