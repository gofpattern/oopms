...
checkText: function (el, myRule) {
    ...
    } else if (myRule.ruleName=='fixlength') {
        if ( isNaN(myRule.comparisonValue) ) {
            yav.debug('DEBUG: comparisonValue for rule ' + myRule.ruleName + ' not a number');
        } else if ( el.value.length > myRule.comparisonValue ) {
            yav.highlight(el, yav_config.inputclasserror);
...
threeParamRule: function (ruleName) {
    return (ruleName=='equal' || ruleName=='notequal' || ruleName=='minlength' || ruleName=='maxlength' ||  ruleName=='fixlength' || ruleName=='date_lt' || ruleName=='date_le' || ruleName=='implies' || ruleName=='regexp' || ruleName=='numrange' || ruleName=='keypress' || ruleName=='and' || ruleName=='or' || ruleName=='custom' || ruleName=='mask')
        ? true
        : false;
},
...
getDefaultMessage: function (el, nameDisplayed, ruleName, comparisonValue) {
    ...
    } else if (ruleName=='fixlength') {
        msg = yav_config.FIXLENGTH_MSG.replace('{1}', nameDisplayed).replace('{2}', comparisonValue);
